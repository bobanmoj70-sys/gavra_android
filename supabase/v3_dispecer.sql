...
-- AŽURIRANO: uklonjen dan_u_sedmici, kapacitet čita iz v3_kapacitet_slots
-- ============================================================
...
BEGIN
  ...
  -- Za svaki zahtev na 'obradi' (osim dnevnih) po redu kreiranja
  FOR v_req IN
    SELECT
      z.id,
      z.grad,
  ...
  LOOP
    ...
    ELSE
      ...
      SELECT to_char(ks.vreme, 'HH24:MI') INTO v_alt1
      FROM v3_kapacitet_slots ks
      WHERE ks.grad   = v_req.grad
        AND ks.datum  = v_req.datum
        AND ks.vreme  < v_req.zeljeno_vreme
      ...
      SELECT to_char(ks.vreme, 'HH24:MI') INTO v_alt2
      FROM v3_kapacitet_slots ks
      WHERE ks.grad   = v_req.grad
        AND ks.datum  = v_req.datum
        AND ks.vreme  > v_req.zeljeno_vreme
    ...
    SET status          = v_novi_status,
        ...
        alt_vreme_pre   = CASE WHEN v_novi_status = 'odbijeno' THEN v_alt1::time ELSE NULL END,
        alt_vreme_posle = CASE WHEN v_novi_status = 'odbijeno' THEN v_alt2::time ELSE NULL END,
        updated_at      = v_now
    WHERE id = v_req.id;

    -- Audit log (bez dan_u_sedmici — koristimo datum)
