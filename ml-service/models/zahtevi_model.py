"""
Zahtevi ML Model - Ensemble (Random Forest + XGBoost)
...
class ZahteviMLModel:
    ...
    def train(self, data: dict):
        ...
        feature_imp = dict(zip(feature_cols, combined_importance)) if self.model_rf else {}
    ...
    def predict_next_week(self, data: dict) -> dict:
        ...
        for grad in grads:
            ...
            for i, d in enumerate(next_week):
                ...
                if XGBOOST_AVAILABLE and self.model_xgb is not None:
                    pred_rf = self.model_rf.predict(X_pred_scaled)
                    pred_xgb = self.model_xgb.predict(X_pred_scaled)
                    pred = (pred_rf + pred_xgb) / 2
                else:
                    pred = self.model_rf.predict(X_pred_scaled)
                
                pred = max(0, pred[0] if isinstance(pred, np.ndarray) else pred)  # Ne može biti negativno
    ...
    def save(self):
        if not self.is_trained:
            return
        joblib.dump(self.model_rf, f"{self.model_dir}/zahtevi_model_rf.pkl")
        if XGBOOST_AVAILABLE and self.model_xgb is not None:
            joblib.dump(self.model_xgb, f"{self.model_dir}/zahtevi_model_xgb.pkl")
        joblib.dump(self.scaler, f"{self.model_dir}/zahtevi_scaler.pkl")
        print("[OK] Zahtevi ensemble model saved")

    def load(self):
        try:
            self.model_rf = joblib.load(f"{self.model_dir}/zahtevi_model_rf.pkl")
            try:
                if XGBOOST_AVAILABLE:
                    self.model_xgb = joblib.load(f"{self.model_dir}/zahtevi_model_xgb.pkl")
            ...
            self.is_trained = True
            print("[OK] Zahtevi ensemble model loaded")
        except FileNotFoundError:
