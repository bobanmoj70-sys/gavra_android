...
class V3JavnaPumpaTocenje {
  final String id;
  final DateTime datumVreme;
  final int? vremeSipanjaMin;
  final double kolicinaL;
  final double iznosRsd;
  final String? vozacId;
  final bool placeno;
  final bool aktivno;
  ...
  V3JavnaPumpaTocenje({
    ...
    this.vremeSipanjaMin,
    required this.kolicinaL,
    required this.iznosRsd,
    this.vozacId,
    this.placeno = false,
    this.aktivno = true,
  });

  factory V3JavnaPumpaTocenje.fromJson(Map<String, dynamic> json) {
    return V3JavnaPumpaTocenje(
      id: json['id']?.toString() ?? '',
      datumVreme: V3DateUtils.parseTs(json['datum_vreme'] as String?) ?? DateTime.now(),
      vremeSipanjaMin: (json['vreme_sipanja_min'] as num?)?.toInt(),
      kolicinaL: (json['kolicina_l'] as num?)?.toDouble() ?? 0,
      iznosRsd: (json['iznos_rsd'] as num?)?.toDouble() ?? 0,
      vozacId: json['vozac_id']?.toString(),
  ...
  Map<String, dynamic> toJson() => {
        if (id.isNotEmpty) 'id': id,
        'datum_vreme': datumVreme.toIso8601String(),
        'vreme_sipanja_min': vremeSipanjaMin,
        'kolicina_l': kolicinaL,
        'iznos_rsd': iznosRsd,
        'vozac_id': vozacId,
        'placeno': placeno,
        'aktivno': aktivno,
