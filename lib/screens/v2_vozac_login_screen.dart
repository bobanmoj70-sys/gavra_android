...
class _VozacLoginScreenState extends State<V2VozacLoginScreen> {
  ...
  Future<void> _login({bool saveBiometric = true}) async {
    ...
    try {
      final vozaci = await _vozacLoadVozaci();
      ...
      if (vozac.isEmpty) {
        V2AppSnackBar.error(context, 'Vozač "${widget.vozacIme}" nije pronađen u sistemu.');
        return;
      }
      ...
      if (vozac['email'].toString().toLowerCase() != email) {
        V2AppSnackBar.error(context, 'Pogrešan email.');
        return;
      }
      ...
      if (normalizedInput != normalizedStored) {
        V2AppSnackBar.error(context, 'Pogrešan broj telefona.');
        return;
      }

      final vozacSifra = vozac['sifra']?.toString() ?? '';
      if (vozacSifra.isNotEmpty && vozacSifra != sifra) {
        V2AppSnackBar.error(context, 'Pogrešna šifra.');
        return;
      }
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: _vozacInputDecoration('Email adresa', Icons.email, isDark, currentTheme),
                  validator: (v) {
                    if (v?.isEmpty == true) {
                ...
                TextFormField(
                  ...
                  decoration: _vozacInputDecoration('Broj telefona', Icons.phone, isDark, currentTheme),
                  validator: (v) {
                ...
                TextFormField(
                  ...
                  decoration: _vozacInputDecoration('Šifra', Icons.lock, isDark, currentTheme).copyWith(
...
Future<List<Map<String, dynamic>>> _vozacLoadVozaci() async {
  try {
    ...
    if (vozaciMaps.isNotEmpty) return vozaciMaps;
  } catch (e) {
    debugPrint('[VozacLoginScreen] _vozacLoadVozaci greška: $e');
  }
  return [];
}

InputDecoration _vozacInputDecoration(
  String label,
  IconData icon,
  bool isDark,
  V2ThemeDefinition currentTheme,
) {
